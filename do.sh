...
options {
    ...
    recursion yes;
    allow-query { any; };
    listen-on { 127.0.0.1; 192.168.41.129; };
    allow-recursion { 192.168.41.0/24; };
...
@       IN      A       192.168.41.129
ns      IN      A       192.168.41.129
www     IN      A       192.168.41.200
api     IN      A       192.168.41.201
fin     IN      A       192.168.41.51
mail    IN      A       192.168.41.14
EOF

# Create Reverse Zone /etc/bind/db.192.168.41
sudo bash -c "cat > /etc/bind/db.192.168.41" <<EOF
\$TTL    604800
@       IN      SOA     ns.ucan.la. admin.ucan.la. (
                         2025051601 ; Serial
                         604800     ; Refresh
...
sudo named-checkzone 41.168.192.in-addr.arpa /etc/bind/db.192.168.41

# Restart BIND9
sudo systemctl restart bind9

echo "✅ DNS Server for ucan.la configured successfully on 192.168.41.129!"
